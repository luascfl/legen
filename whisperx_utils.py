...
def transcribe_audio(model: whisperx.asr.WhisperModel, audio_path: Path, srt_path: Path, lang: str = None, device: str = "cpu", batch_size: int = 4):
    ...
    if lang in whisperx.alignment.DEFAULT_ALIGN_MODELS_HF or lang in whisperx.alignment.DEFAULT_ALIGN_MODELS_TORCH:
        with time_task(message_start="Running alignment...", end='\n'):
            try:
                model_a, metadata = whisperx.load_align_model(language_code=lang, device="cuda", use_auth_token=os.environ["HUGGINGFACE_TOKEN"])
                ...
                model_a, metadata = whisperx.load_align_model(language_code=lang, device="cpu", use_auth_token=os.environ["HUGGINGFACE_TOKEN"])  # force load on cpu due to errors on gpu
                transcribe = whisperx.align(
                    transcript=transcribe["segments"],
                    model=model_a,
                    align_model_metadata=metadata,
